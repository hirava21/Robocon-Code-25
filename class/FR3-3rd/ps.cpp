...
void PS::update() {
  ...
  while (Serial5.available()) {
    ...
    if (cmd == "RELAY 1") buttonLeft  = 1;  // single ↓
    if (cmd == "RELAY2") buttonLeft  = 2;  // double ↓
    // — Face buttons
...
void PS::resetPulses() {
  // clear D-pad counts
  dpadUpCount = dpadRightCount = dpadLeftCount = dpadDownCount = buttonLeft= 0;
  // clear face-button one-shots
  buttonUp = buttonDown  = buttonRight = false;
  l1 = r1 = false;
}
