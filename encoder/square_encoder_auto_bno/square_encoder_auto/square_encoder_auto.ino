...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(2), ai0FL, RISING);
  attachInterrupt(digitalPinToInterrupt(3), ai0FR, RISING);
...
void square(){
  ...
  stop();
  updatePosition();
  Serial.println("Square motion complete, returning to start position.");
  delay(5000);
}