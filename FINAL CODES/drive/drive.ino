...
if (abs(joyY) < 5) joyY = 0;
 ...
 float error = target_Yaw - actual_Yaw;
    float integral += error;
    float derivative = error - prevError;
    float prevError = error;

    float correction_Yaw = (Kp * error) + (Ki * integral) + (Kd * derivative);
    
    // // Clamping output to min/max range
    if (correction > maxOutput) output = 180;
    if (output < minOutput) output = -180;

    return output;

    Serial.print(" | Correction Yaw: "); Serial.print(correct_Yaw);
