...
void pidControl() {
    ...
    myPID.Compute();
    rotationSpeed = constrain(abs(Output), 0, 150);
    ...
    if (Output > 2) {
        ...
        anticlockwise();
        Serial.println("anticlck");  // Fixed: Changed `printn` to `println`
    }
     else {
        stop();
        Serial.println("stop");
...
void clockwise() {
    digitalWrite(DIR1, HIGH); analogWrite(PWM1, rotationSpeed);
    digitalWrite(DIR2, LOW); analogWrite(PWM2, rotationSpeed);
    digitalWrite(DIR3, LOW); analogWrite(PWM3, rotationSpeed);
    digitalWrite(DIR4, HIGH); analogWrite(PWM4, rotationSpeed);
}
void anticlockwise() {
    digitalWrite(DIR1, LOW); analogWrite(PWM1, rotationSpeed);
    digitalWrite(DIR2, HIGH); analogWrite(PWM2, rotationSpeed);
    digitalWrite(DIR3, HIGH); analogWrite(PWM3, rotationSpeed);
    digitalWrite(DIR4, LOW); analogWrite(PWM4, rotationSpeed);
}