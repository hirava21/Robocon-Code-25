...
int BS = 150;  // Peak speed
int currentBS = 0;  // Starting speed
int incrementStep = 50;  // Speed increment per cycle
int incrementDelay = 200;  // Delay in ms between increments
...
double Kp = 1.5, Ki = 0.5, Kd = 0; // Tune these values for best performance
...
void loop() {
  if (myIMU.dataAvailable()) {
    ...
    if (millis() - lastIncrementTime >= incrementDelay && currentBS < BS) {
      currentBS += incrementStep;
      if (currentBS > BS) currentBS = BS;
    ...
    Serial.print(yawOutput, 2);
    Serial.print(" | Speed: ");
    Serial.println(currentBS);

    int leftSpeed = currentBS - yawOutput;
    int rightSpeed = currentBS + yawOutput;
    int leftBack = currentBS - yawOutput;
    int rightBack = currentBS + yawOutput;

    // Ensure values stay within motor limits
    leftSpeed = constrain(leftSpeed, 0, 255);
    rightSpeed = constrain(rightSpeed, 0, 255);
    leftBack = constrain(leftBack, 0, 255);
    rightBack = constrain(rightBack, 0, 255);
    if (isMovingForward) forward(leftSpeed, rightSpeed, leftBack, rightBack);
    else reverse(leftSpeed, rightSpeed, leftBack, rightBack);
  }
}

// Moves the robot forward
void forward(int LS, int RS, int BL, int BR) {
  digitalWrite(FL_dir, 1);
  analogWrite(FL_pwm, LS);
  digitalWrite(FR_dir, 1);
  analogWrite(FR_pwm, RS);
  digitalWrite(RR_dir, 1);
  analogWrite(RR_pwm, BR);
  digitalWrite(RL_dir, 1);
  analogWrite(RL_pwm, BL);
}

// Moves the robot backward
void reverse(int LS, int RS, int BL, int BR) {
